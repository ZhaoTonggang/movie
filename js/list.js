...
const urldata = window.location.href,
	Config = { // 分类配置
		dianying: {
			...
			n: ['全部', 2026, 2025, 2024, 2023, 2022, 2021, 2020, 2019, 2018, 2017, 2016, 2015, 2014, 2013, 2012, 2010,
				2009, 2008, 2007, '更早'
			],
	...
	getmov = (s) => { // 获取内容列表
		const listList = document.getElementById('listList'),
			pxv = '&rank=' + radio('paixu');
		listList.innerHTML =
			'<div class="s-loading"><i class="first"></i><i class="second"></i><i class="third"></i></div>';
		let lxv = tegId.l ? '&cat=' + filter(radio('lx')) : '',
			ndv = tegId.n ? '&year=' + filter(radio('nd')) : '',
			dqv = tegId.d ? '&area=' + filter(radio('dq')) : '',
			mxv = tegId.m ? '&act=' + filter(radio('mx')) : '',
			pgv = '';
		if (s == 1) {
			page += 1;
		}
		...
		post('filter', 'catid=' + urlId + pxv + lxv + ndv + dqv + mxv + pgv).then(datas => {
			if (datas.code == 1) {
				...
				for (let i = 0; i < len; i++) {
					// 根据不同分类显示不同的标签
					let tagContent = '',
						coverData = /https:\/\//i.test(datas[i].cdncover) ? datas[i].cdncover : ('https:' +
							datas[i].cdncover);
					if (urlId == '1') {
						// 电影显示评分
